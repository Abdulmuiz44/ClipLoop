      ...
      <div className="grid gap-4 md:grid-cols-2">
        {[
          ...
          <motion.div
            ...
            className="rounded-2xl border border-[#1F1F1F] bg-[#0E0E0E] p-5 shadow-sm border-[#1F1F1F] bg-[#0E0E0E]"
          >
            <div className="flex items-start justify-between">
              <h3 className="text-lg font-semibold text-white text-white">{item.title}</h3>
              {item.coming && (
                <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[11px] font-medium text-amber-700 bg-amber-500/15 text-amber-300">
                  Coming Soon
                </span>
              )}
            </div>
            <p className="mt-2 text-sm text-[#8B8B8B] text-[#8B8B8B]">{item.desc}</p>
          </motion.div>
        ))}
      </div>
    </motion.div>
