...
export function AccessRequestForm({
  ...
  return (
    <form action={onSubmit} className={`cl-card space-y-4 p-5 md:p-6 ${className}`}>
      ...
      {message ? <p className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-3 text-sm leading-6 text-slate-700">{message}</p> : null}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {showBackHome ? (
          <Link href="/" className="inline-flex items-center rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-700 transition hover:border-slate-400">
            Back to home
          </Link>
        ) : null}
      </div>
