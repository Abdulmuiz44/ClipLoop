...
import { bulkScheduleStrategyCycleContent } from "@/domains/publishing/service";
...
export async function POST(request: Request, context: { params: Promise<{ strategyCycleId: string }> }) {
  try {
    ...
    const body = bulkScheduleBodySchema.parse(await request.json());
    ...
    const result = await bulkScheduleStrategyCycleContent(strategyCycleId, body);
