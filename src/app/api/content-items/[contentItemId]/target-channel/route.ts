...
import { updateContentItemTargetChannel } from "@/domains/content-items/service";
import { updateContentItemTargetChannelBodySchema } from "@/lib/validation/content";
...
export async function PATCH(request: Request, context: { params: Promise<{ contentItemId: string }> }) {
  try {
    ...
    const body = updateContentItemTargetChannelBodySchema.parse(await request.json());
    ...
    const updated = await updateContentItemTargetChannel(contentItemId, body.targetChannel);
