export function exportForX({ storyboard, renderPath }) {
  const title = storyboard?.title || "ClipLane v0.1.0";
  const post = [
    `${title}`,
    "",
    "ClipLane is the open-source, local-first workflow layer for turning product updates into short-form promo videos.",
    "",
    `Video: ${renderPath}`,
    "",
